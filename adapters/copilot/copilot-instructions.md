...
- Bulletproof Backend Architecture: Validate environment at startup (Zod/Pydantic BaseSettings), scope all DB queries to verified userId (anti-IDOR), prevent N+1 queries with eager loading, enforce pagination limits (max 50-100), wrap multi-step writes in atomic transactions, use strict input schemas (anti-mass assignment), and sanitize production errors with correlation IDs.
