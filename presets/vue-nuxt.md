# Preset: Vue & Nuxt Low-CPU Optimization
...
- Use `shallowRef()` or `shallowReactive()` for large arrays or objects when deep reactivity is not required. Deep proxy wrapping on large lists wastes CPU cycles.
